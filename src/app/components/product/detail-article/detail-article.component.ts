...
export class DetailArticleComponent implements OnInit {
  ...
  clicked6: boolean;
  fileName: string = '';
  fileAbsolutUrl: string =
    'http://localhost:8080/upload/static/images/products/';
  imageAbsolutUrl = '';
      ...
      this.productServ.getProductById(this.id).subscribe((data) => {
        ...
        if (this.product.fileVector != null && this.product.fileVector != '') {
          ...
          this.filesAvailable.push(
            this.fileAbsolutUrl + 'file/' + this.product.fileVector
          );
        }
        if (this.product.filePDF != null && this.product.filePDF != '') {
          ...
          this.clicked2 = true;
          this.filesAvailable.push(
            this.fileAbsolutUrl + 'file/' + this.product.filePDF
          );
        }
        if (this.product.fileIcons != null && this.product.fileIcons != '') {
          ...
          this.clicked3 = true;
          this.filesAvailable.push(
            this.fileAbsolutUrl + 'file/' + this.product.fileIcons
          );
        }
        if (this.product.fileEPS != null && this.product.fileEPS != '') {
          ...
          this.clicked4 = true;
          this.filesAvailable.push(
            this.fileAbsolutUrl + 'file/' + this.product.fileEPS
          );
        }
        if (this.product.fileSVG != null && this.product.fileSVG != '') {
          ...
          this.clicked5 = true;
          this.filesAvailable.push(
            this.fileAbsolutUrl + 'file/' + this.product.fileSVG
          );
        }
        if (this.product.filePNG != null && this.product.filePNG != '') {
          ...
          this.clicked6 = true;
          this.filesAvailable.push(
            this.fileAbsolutUrl + 'file/' + this.product.filePNG
          );
        }
        ...
        this.productServ
          .findProductSameIllustration(
            this.product.colorP,
            this.product.formatP,
            this.product.styleGraphicP
          )
          .subscribe((dataIllustration) => {
            ...
            for (let i = 0; i < this.productIllustration.length; i++) {
              this.productIllustration[i].imageP =
                'http://localhost:8080/upload/static/images/products/' +
                this.productIllustration[i].imageP;
            }
          });
